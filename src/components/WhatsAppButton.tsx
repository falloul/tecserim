...
const WhatsAppButton = () => {
  ...
  const content = getContent(language).contact.info;
  
  // Format phone number by removing spaces and special characters
  const phoneNumber = content.gsm?.replace(/\s+/g, '').replace(/[\+\-\(\)]/g, '') || 
                      content.tel?.replace(/\s+/g, '').replace(/[\+\-\(\)]/g, '');
  ...
  return (
    ...
    >
      ...
      >
        {/* WhatsApp icon path from Font Awesome */}
